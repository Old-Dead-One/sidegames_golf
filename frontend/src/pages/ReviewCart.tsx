...
const ReviewCart: React.FC<ReviewCartProps> = ({ theme, onOpenCart }) => {
    ...
    const handleCheckout = async () => {
        const { data: { session } } = await supabase.auth.getSession();
        if (!session) {
            toast.error("You must be logged in to complete your purchase. Please sign in or create an account.");
            return;
        }
        const userId = session.user.id;
        ...
        try {
            ...
            // Clear the cart in the database (or create empty cart if none exists)
            ...
            if (error) {
                ...
                // Don't throw error - cart clearing is not critical for purchase completion
            }
